...
Load and resolve custom file types at runtime with Node.js's classic `require` or next-generation
...
const result = require('./some/non-js/file');
```
...
`--experimental-loader` and a [supported loader type](#supported-file-types).
...
The TypeScript ESM loader can be referenced with `@boost/module/typescript.mjs`.
