...
next-generation [hooks](https://nodejs.org/api/module.html#customization-hooks). Currently supports
...
[hooks](https://nodejs.org/api/module.html#customization-hooks), where non-JavaScript files can be
...
If you meet all of these requirements, then you may run your Node.js script with one of the
following patterns.
...
[TypeScript](https://www.typescriptlang.org/) files are supported for `.ts`, `.tsx`, `.cts`, and
`.mts` file extensions. The TypeScript source code will be down-leveled according to the currently
