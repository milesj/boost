...
[Command](/api/cli/class/Command) class, and implement a `run()` method. This method can be async
and even render [React components](#rendering-components)!
...
type safety, the [Params](/api/cli#Params) collection type can be used to type the static property.
