...
[middleware](#middleware) to argv (`process.argv`), parsing argv into [arguments](./args.mdx#usage)
...
property, which requires a mapping of option names to [option settings](./args.mdx#options). With
...
property, which requires an array of [param settings](./args.mdx#params).
...
[Rest arguments](./args.mdx#rest) are all arguments that come after a standalone `--` delimiter, and
