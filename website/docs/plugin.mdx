...
class, which requires a project name (used as a [package prefix and scope](#naming-guidelines)),
...
- `afterShutdown` (`async (plugin: T) => void`) - Callback fired after a plugin's `shutdown` life
  cycle is executed.
- `resolver` (`(id: string) => string`) - Custom module resolver instead of `require.resolve`.
